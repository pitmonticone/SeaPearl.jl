include("cp_layer_coloring/cp_layer.jl")

"""
    ColoringStateRepresentation{F, TS}

Representation bipartite for the graph coloring problem (only variables and values)

It consists in a bipartite graph representation of the CP Model, with features associated with each node
and an index specifying the variable that should be branched on.
"""
mutable struct ColoringStateRepresentation{F, TS} <: FeaturizedStateRepresentation{F, TS}
    cplayergraph::CPLayerGraph2
...
function ColoringStateRepresentation{F, TS}(model::CPModel; action_space=nothing) where {F, TS}
    g = CPLayerGraph2(model)
    allValuesIdx = nothing
    if !isnothing(action_space)
        allValuesIdx = indexFromCpVertex.([g], ValueVertex.(action_space))
    end
    sr = ColoringStateRepresentation{F, TS}(g, nothing, nothing, nothing, allValuesIdx)
    sr.nodeFeatures = featurize(sr)
    sr.globalFeatures = global_featurize(sr)
    return sr
end

function DefaultTrajectoryState(sr::ColoringStateRepresentation{F, DefaultTrajectoryState}) where F
...
"""
function update_representation!(sr::ColoringStateRepresentation, model::CPModel, x::AbstractIntVar)
...
"""
feature_length(::Type{<:FeaturizedStateRepresentation{DefaultFeaturization, TS}}) where TS = 2

ColoringStateRepresentation(m::CPModel) = ColoringStateRepresentation{DefaultFeaturization, DefaultTrajectoryState}(m::CPModel)

...
function fill_with_generator!(cpmodel::CPModel, gen::Eternity2Generator; seed=nothing)
    ...
    for i = 1:n
        src_v[i,m+1] = SeaPearl.IntVar(0, k, "srcv_"*string(i)*string(m+1), cpmodel.trailer)
        SeaPearl.addVariable!(cpmodel, src_v[i,m+1]; branchable=false)
    end
    for j =1:m
        src_h[n+1,j] = SeaPearl.IntVar(0, k, "srch_"*string(n+1)*string(j), cpmodel.trailer)
    ...
    for i = 1:n, j=1:m
        ...
        src_v[i,j] = SeaPearl.IntVar(0, k, "srcv_"*string(i)*string(j), cpmodel.trailer)
        SeaPearl.addVariable!(cpmodel, src_v[i,j]; branchable=false)
        src_h[i,j] = SeaPearl.IntVar(0, k, "srch_"*string(i)*string(j), cpmodel.trailer)
